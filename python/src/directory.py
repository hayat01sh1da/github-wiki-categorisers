...
class Directory(Application):
    ...
    def run(self):
        ...
        print(self.source_path_maps)
        self.__target_dirs__()
        print(self.target_dirs)
        self.__mkdirs__()
    ...
    def __delete_empty_dirs__(self):
        ...
        for dir in dirs:
            ...
            elif glob.glob(os.path.join(dir, '**', '*.md')) or glob.glob(os.path.join(dir, '**', '*.png')):
                continue
            # [NOTE] Python files for automation are needed.
            elif glob.glob(os.path.join(dir, '**', '*.py')):
                continue
            else:
                shutil.rmtree(dir)
